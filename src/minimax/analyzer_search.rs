...
use crate::minimax::{Score, ttable, eval};
    ...
    for mv in &my_moves {
        ...
        for depth in 1..target_depth {
            ...
            while bounds[0] < bounds[1] {
                ...
                guess = full_width_alphabeta(&board, &mut node_counter, deadline, *mv, &mut enemy_moves, &mut history, depth, beta-1, beta).unwrap();
...
pub fn full_width_alphabeta<const S: usize, const W: usize, const H: usize, const WRAP: bool, const HZSTACK: bool>(
    board: &Bitboard<S, W, H, WRAP, HZSTACK>,
        ...
        let score = 'max_call: { // max call
            ...
            *node_counter += 1;
            ...
            for mv in itt_move.iter().chain(ordered_allowed_moves(&child, 0, history).iter()) {
                ...
                let iscore = full_width_alphabeta(&child, node_counter, deadline, *mv, &mut next_enemy_moves, history, depth-1, ialpha, ibeta)?;
                if iscore > ibeta {
                    ibest_score = iscore;
                    ibest_move = *mv;
                    break;
